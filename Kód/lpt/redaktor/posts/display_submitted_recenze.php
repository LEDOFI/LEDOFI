...
<html lang="en">
    ...
    <head>
        ...
        <title>Redaktor - Zobrazit článek</title>
    </head>
	...
	$results = mysqli_query($conn," SELECT * FROM posts, posts_assets, posts_autors, posts_recenze
                                    WHERE posts.id=posts_autors.id AND posts.id=posts_assets.id AND posts.id=posts_recenze.id AND posts_autors.verze=posts_assets.verze AND posts_autors.verze=posts_recenze.verze AND posts.id='$id' AND posts_recenze.recenzent_id='$recenzent_id' AND posts_autors.verze='$verze'
        ...
        <div class="main-content single">
          ...
          <div class="post-content">
	        ...
	        $results_2 = mysqli_query($conn," SELECT * FROM posts, posts_assets,  posts_autors
                                    WHERE posts.id=posts_autors.id AND posts.id=posts_assets.id AND posts_assets.verze=posts_autors.verze AND posts_assets.verze='$postVERZE' AND posts.id='$id'
                                    ORDER BY posts_assets.verze"); // select query
			...
			$results_3 = mysqli_query($conn," SELECT * FROM users, posts_recenze WHERE users.id='$recenzent_id' AND posts_recenze.verze='$postVERZE' AND posts_recenze.id='$id'"); // select query
			while ($recenzent = mysqli_fetch_assoc($results_3)) {
			
				echo "<br><br><div align='right'>Jméno recenzenta: " . $recenzent['username'] . "</div><br>";
			}	
			
			$results_4 = mysqli_query($conn," SELECT * FROM posts_recenze WHERE posts_recenze.verze='$postVERZE' AND posts_recenze.id='$id'"); // select query
