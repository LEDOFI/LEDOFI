...
<html lang="en">
    ...
    if ((isset($_GET['id'])) && (isset($_GET['verze'])) && (isset($_GET['verze']))) {

  	$id = $_GET['id'];
	$verze = $_GET['verze'];
	$recenzentID = $_GET['recenzent_id'];

	$results = mysqli_query($conn," SELECT * FROM posts, posts_assets, posts_autors, posts_recenze, posts_vybrani_recenzenti
                                    WHERE posts.id=posts_autors.id AND posts.id=posts_assets.id AND posts.id=posts_recenze.id AND posts.id=posts_vybrani_recenzenti.id AND posts_autors.verze=posts_assets.verze AND posts_autors.verze=posts_recenze.verze AND posts_autors.verze=posts_vybrani_recenzenti.verze AND posts.id='$id' AND posts_recenze.verze='$verze' AND posts_recenze.recenzent_id='$recenzentID' AND posts_vybrani_recenzenti.zpristupneno_autorovi='1'
      ...
      <div class="main-content-wrapper">
        <div class="main-content single">
          ...
          <div class="post-content">
			...
			$results_3 = mysqli_query($conn," SELECT * FROM users WHERE id='$recenzentID'");
			while ($recenzent = mysqli_fetch_assoc($results_3)) {
			
				echo "<br><br><div align='right'>Jméno recenzenta: " . $recenzent['username'] . "</div><br>";
			}	
			
			$results_4 = mysqli_query($conn," SELECT * FROM posts_recenze WHERE posts_recenze.verze='$postVERZE' AND posts_recenze.id='$id' AND posts_recenze.recenzent_id='$recenzentID'");
		  ...
		  <a href="display_submitted_recenze_all_versions.php?id=<?php echo $post['id']; ?>" class="display">Zobrazit všechny recenze pro všechny verze tohoto příspěvku.</a>
